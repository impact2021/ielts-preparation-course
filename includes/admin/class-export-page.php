...
class IELTS_CM_Export_Page {
    ...
    public function render_export_page() {
        ...
        // Get counts for each post type with null safety
        $course_counts = wp_count_posts('ielts_course');
        $lesson_counts = wp_count_posts('ielts_lesson');
        $resource_counts = wp_count_posts('ielts_resource');
        $quiz_counts = wp_count_posts('ielts_quiz');
        
        $course_count = isset($course_counts->publish) ? $course_counts->publish : 0;
        $lesson_count = isset($lesson_counts->publish) ? $lesson_counts->publish : 0;
        $resource_count = isset($resource_counts->publish) ? $resource_counts->publish : 0;
        $quiz_count = isset($quiz_counts->publish) ? $quiz_counts->publish : 0;
