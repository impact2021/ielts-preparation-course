...
class IELTS_CM_Admin {
    ...
    public function add_admin_menu() {
        ...
        // Add Hybrid Site Settings menu if hybrid mode is enabled
        if (get_option('ielts_cm_hybrid_site_enabled', false)) {
            add_submenu_page(
                ...
                array($this, 'hybrid_settings_page')
            );
        }
    }
...
text: '&lt;h3&gt;Welcome to IELTS!&lt;/h3&gt;&lt;p&gt;Your learning journey begins here...&lt;/p&gt;'
    ...
    public function payment_errors_page() {
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'clear_all' && check_admin_referer('clear_all_payment_errors')) {
            // Note: Table name is trusted (WordPress prefix + plugin table name)
            // WordPress doesn't support table name parameters in prepare(), so we use esc_sql
            // The table name is constructed internally and not user-provided
     ...
     * Shows payment settings for both partners (to purchase codes) and students (to purchase course extensions)
     */
    public function hybrid_settings_page() {
        // Check user capability
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'ielts-course-manager'));
        }
        ...
        if (isset($_POST['ielts_cm_hybrid_settings_nonce']) && wp_verify_nonce($_POST['ielts_cm_hybrid_settings_nonce'], 'ielts_cm_hybrid_settings')) {
            ...
            // Save access code pricing
            $code_pricing = array();
            if (isset($_POST['access_code_50'])) {
                $price = floatval($_POST['access_code_50']);
                if ($price <= 0) {
                    $errors[] = __('50 Access Codes must have a price greater than $0.', 'ielts-course-manager');
                    $price = 50.00;
                }
                $code_pricing['50'] = $price;
            }
            if (isset($_POST['access_code_100'])) {
                $price = floatval($_POST['access_code_100']);
                if ($price <= 0) {
                    $errors[] = __('100 Access Codes must have a price greater than $0.', 'ielts-course-manager');
                    $price = 90.00;
                }
                $code_pricing['100'] = $price;
            }
            if (isset($_POST['access_code_200'])) {
                $price = floatval($_POST['access_code_200']);
                if ($price <= 0) {
                    $errors[] = __('200 Access Codes must have a price greater than $0.', 'ielts-course-manager');
                    $price = 170.00;
                }
                $code_pricing['200'] = $price;
            }
            if (isset($_POST['access_code_300'])) {
                $price = floatval($_POST['access_code_300']);
                if ($price <= 0) {
                    $errors[] = __('300 Access Codes must have a price greater than $0.', 'ielts-course-manager');
                    $price = 240.00;
                }
                $code_pricing['300'] = $price;
            }
            update_option('ielts_cm_access_code_pricing', $code_pricing);
            ...
            if (!empty($errors)) {
                ...
                echo '</ul><p>' . __('Default pricing has been set. Please update to the correct prices.', 'ielts-course-manager') . '</p></div>';
            } else {
                echo '<div class="notice notice-success is-dismissible"><p>' . __('Settings saved.', 'ielts-course-manager') . '</p></div>';
            }
        }
        ...
        // Get access code pricing with defaults
        ...
        <div class="wrap">
            ...
            <form method="post" action="">
                ...
                <p class="description"><?php _e('Set the prices partners pay to purchase access codes in bulk. Partners will be able to purchase these code packages and distribute them to their students.', 'ielts-course-manager'); ?></p>
                <table class="form-table">
                    <tr>
                        <th scope="row"><?php _e('50 Access Codes', 'ielts-course-manager'); ?></th>
                        <td>
                            <input type="number" step="0.01" min="0" name="access_code_50" value="<?php echo esc_attr($code_pricing['50'] ?? 50.00); ?>" class="regular-text">
                            <p class="description"><?php _e('Price in USD (default: $50)', 'ielts-course-manager'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><?php _e('100 Access Codes', 'ielts-course-manager'); ?></th>
                        <td>
                            <input type="number" step="0.01" min="0" name="access_code_100" value="<?php echo esc_attr($code_pricing['100'] ?? 90.00); ?>" class="regular-text">
                            <p class="description"><?php _e('Price in USD (default: $90)', 'ielts-course-manager'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><?php _e('200 Access Codes', 'ielts-course-manager'); ?></th>
                        <td>
                            <input type="number" step="0.01" min="0" name="access_code_200" value="<?php echo esc_attr($code_pricing['200'] ?? 170.00); ?>" class="regular-text">
                            <p class="description"><?php _e('Price in USD (default: $170)', 'ielts-course-manager'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><?php _e('300 Access Codes', 'ielts-course-manager'); ?></th>
                        <td>
                            <input type="number" step="0.01" min="0" name="access_code_300" value="<?php echo esc_attr($code_pricing['300'] ?? 240.00); ?>" class="regular-text">
                            <p class="description"><?php _e('Price in USD (default: $240)', 'ielts-course-manager'); ?></p>
                        </td>
                    </tr>
                </table>
