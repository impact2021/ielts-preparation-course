...
class IELTS_CM_Writing_Assessment {
    ...
    private function apply_wholly_irrelevant_cap($assessment) {
        ...
        $flag = filter_var($flag_raw, FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE);
        $is_wholly_irrelevant = ($flag === null) ? !empty($flag_raw) : $flag;
...
- Band 9 does not require a perfect essay — it requires an essay with no meaningful weaknesses. However, "no meaningful weaknesses" is a high bar. Minor stylistic preferences (such as avoiding "In conclusion") are not weaknesses. Noticeably repetitive or intrusive linking, hedging language, or ideas that are well-developed but not fully extended can all be genuine reasons to withhold Band 9. Award Band 9 only when you cannot identify a single meaningful weakness across the criterion.
...
- For Coherence & Cohesion at Band 8.5 vs Band 9: Band 9 requires cohesion to feel natural and unobtrusive overall, but this does not mean every linking phrase must be invisible. In a concise Task 2 essay, a small number of clear signpost phrases can still be fully appropriate at Band 9 if progression is smooth and meaning is never strained. Award Band 8.5, not Band 9, only when the linking becomes noticeably repetitive, templated, illogical, or so prominent that it replaces genuine flow.
