...
Updated the replacement logic in `includes/admin/class-admin.php` (lines 2279-2291) to accept both double and triple underscores with precise matching:
...
This approach:
- **Checks for triple underscores first**: Prefers the original documented format
- **Falls back to double underscores**: Supports the alternative format
...
2. **includes/admin/class-admin.php** (11 lines changed)
   - Line 1733: Updated instruction text to mention both underscore formats
   - Line 2269: Updated comment to reflect both formats
   - Lines 2279-2291: Updated replacement logic to accept both `__N__` and `___N___` with precise matching
...
**Total**: 5 files modified, 26 insertions, 7 deletions
...
The changes affect only 5 files with 26 insertions and 7 deletions, maintaining the principle of minimal modifications while solving the reported issue completely.
